...
export function registerHandlers(bot: Telegraf) {
  bot.command("start", async (ctx) => {
    ...
    if (!chatId) {
      // If there's no chat ID, we can't reply normally using Telegraf's context
      console.error("Received a start command but chat.id is missing.");
      return;
    }

    // Ensure isOwner handles numbers vs strings correctly depending on your auth logic
    if (!isOwner(chatId)) {
      console.log(`Unauthorized access attempt by chat ID: ${chatId}`);
      return;
    }
