...
export class ToolExecuter {
  ...
  private resolvePath(relPath: string): string {
    const abs = path.resolve(this.config.codebasePath, relPath);
    const root = path.resolve(this.config.codebasePath);
    const relative = path.relative(root, abs);
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new Error(`Path ${relPath} is outside of the codebase root ${root}`);
    }
    return abs;
  }
  ...
  searchFiles(
    ...
    const rootAbs = this.resolvePath(rootRel);
  ...
  applyApprovedFromTracker(): { errors: string[] } {
    ...
    for (const a of all.filter(
      ...
      try {
        fs.mkdirSync(this.resolvePath(a.path), { recursive: true });
      } catch (e) {
    ...
    for (const [p, a] of lastByPath) {
      try {
        if (a.type === "file_delete")
          fs.rmSync(this.resolvePath(p), { force: true });
        else {
